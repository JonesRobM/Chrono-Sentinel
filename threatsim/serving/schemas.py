...
class UncertaintyInterval(BaseModel):
    ...
    lower: float = Field(..., description="Point estimate minus two standard deviations, clipped to 0")
    upper: float = Field(..., description="Point estimate plus two standard deviations, clipped to 1")
...
class ScoreResponse(BaseModel):
    ...
    model_version: str = Field(..., description="Short content hash of the served checkpoint")
    inference_ms: float = Field(..., description="Server-side forward-pass time in milliseconds")
...
class ReadyResponse(BaseModel):
    ...
    default_mc_samples: Optional[int] = None
